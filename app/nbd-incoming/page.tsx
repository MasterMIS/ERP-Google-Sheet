...
function NBDContent() {
    ...
    const fetchNBDs = async () => {
        try {
            const res = await fetch('/api/nbd-incoming');
            if (res.ok) {
                ...
                toast.error('Failed to fetch NBD Incoming records');
            }

            // Fetch follow-up data
            const followUpRes = await fetch('/api/nbd-incoming-followup');
            if (followUpRes.ok) {
    ...
    const fetchNBDHistory = async (id: number) => {
        try {
            const res = await fetch(`/api/nbd-incoming-followup?id=${id}`);
            if (res.ok) {
                const data = await res.json();
                setNbdHistory(data);
            }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.ok) {
                toast.success(isEditing ? 'NBD Incoming updated' : 'NBD Incoming created');
    ...
    const confirmDelete = async () => {
        ...
        try {
            loader.showLoader();
            const res = await fetch(`/api/nbd-incoming?id=${deleteId}`, { method: 'DELETE' });
            if (res.ok) {
    ...
    const handleBulkShiftToCRR = async () => {
        ...
        console.log('🔍 Shift to CRR Debug Info:');
        console.log('  Total NBDs in state:', nbds.length);
        console.log('  Eligible IDs (order_won_count >= 3):', eligibleIds);
        console.log('  Eligible count:', eligibleIds.length);
        ...
        try {
            loader.showLoader();
            toast.info(`Attempting to shift ${eligibleIds.length} record(s) to CRR...`);
            ...
            if (res.ok) {
                const data = await res.json();
                console.log('✅ Shift successful:', data);
                toast.success(data.message || `Shifted ${eligibleIds.length} record(s) to CRR successfully`);
                fetchNBDs();
            } else {
                const data = await res.json();
                console.error('❌ Shift failed:', data);
                console.error('  Attempted IDs:', eligibleIds);
            ...
            console.error('  Attempted IDs:', eligibleIds);
            toast.error('Error shifting records. Check console for details.');
        } finally {
            loader.hideLoader();
        }
    };
    ...
    const handleImportCSV = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            if (res.ok) {
                toast.success(`Successfully imported ${records.length} NBD Incoming records`);
                fetchNBDs();
            } else {
            ...
            <div className="p-4 max-w-[1920px] mx-auto">
                ...
                >
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                        <div>
                            <h1 className="text-xl sm:text-2xl font-bold text-[var(--theme-primary)] dark:text-[var(--theme-primary)] mb-2">
                                NBD Incoming Management
                            </h1>
                        ...
                        <div className="flex gap-2 flex-wrap items-center">
                            ...
                            <motion.button
                                ...
                                title="Show NBD Incoming records with today's or past follow-up dates"
                            >
                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            >
                                ...
                                <span className="hidden sm:inline">Add New NBD Incoming</span>
                    ...
                    >
                        {sortedNBDs.length === 0 ? (
                            <div className="p-12 text-center">
                                ...
                                <p className="text-gray-600 dark:text-gray-400 mb-4">No NBD Incoming records found</p>
                                <button
                                    onClick={() => openModal()}
                                    className="text-[var(--theme-primary)] hover:underline font-semibold"
                                >
                                    Create your first NBD Incoming
                                </button>
                            </div>
                        ) : (
                            <>
                                            ...
                                            <table className="w-full">
                                                ...
                                                <tbody className="divide-y divide-gray-200 dark:divide-gray-700">
                                                    {paginatedNBDs.map((nbd) => {
                                                        const followUp = followUpDataMap.get(Number(nbd.id));
                                                        const isOrderWon = (followUp?.order_won_count || 0) >= 3;
                                                        return (
                                                            <motion.tr
                                                                key={nbd.id}
                                                                className={`transition cursor-pointer ${isOrderWon ? "bg-green-200/40 hover:bg-green-200/60 dark:bg-green-900/10 dark:hover:bg-green-900/20" : "hover:bg-[var(--theme-lighter)]/50 dark:hover:bg-gray-700/50"}`}
                                            ...
                                            {paginatedNBDs.map((nbd, index) => {
                                                ...
                                                return (
                                                    <motion.div
                                                        ...
                                                        className={`bg-gradient-to-br from-white to-[var(--theme-light)] dark:from-gray-800 dark:to-gray-850 rounded-xl shadow-md hover:shadow-xl transition-all duration-300 border border-gray-200 dark:border-gray-700 hover:border-[var(--theme-primary)] overflow-hidden ${isOrderWon ? "bg-green-200/60 dark:bg-green-900/40" : ""}`}
                                                    >
                                                        <div className="p-4">
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        <motion.button
                                            onClick={async () => {
                                                ...
                                                try {
                                                    loader.showLoader();
                                                    const res = await fetch('/api/nbd-incoming-followup', {
                                                        method: 'POST',
                                                        headers: { 'Content-Type': 'application/json' },
                                                        body: JSON.stringify({
                                                            nbd_id: selectedNBD.id,
                            ...
                            >
                                ...
                                <div className="bg-gradient-to-r from-[var(--theme-primary)] to-[var(--theme-secondary)] p-6 sticky top-0 z-10">
                                    <div className="flex justify-between items-center">
                                        <h2 className="text-2xl font-bold text-gray-900">
                                            {editingNBD ? 'Edit NBD Incoming' : 'Create New NBD Incoming'}
                                        </h2>
                                        <button
                                            onClick={() => setIsModalOpen(false)}
                                            className="p-2 hover:bg-white/20 rounded-lg transition"
                                ...
                                <form onSubmit={handleSubmit} className="p-6 space-y-5">
                                    ...
                                    >
                                        ...
                                        >
                                            {editingNBD ? 'Update NBD Incoming' : 'Create NBD Incoming'}
                                        </button>
                                        <button
                                            type="button"
                                            onClick={() => setIsModalOpen(false)}
                                    ...
                                    <div className="flex items-start gap-4 mb-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                                                Are you sure you want to delete this NBD Incoming record?
                                            </p>
                                        </div>
                                    </div>
