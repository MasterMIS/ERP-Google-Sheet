...
import { getGoogleSheetsClient, getCRRs } from '@/lib/sheets';
...
export async function POST(request: Request) {
    try {
        ...
        // Fallback to CRR's main data if not provided (renamed from NBD for context)
        if (!final_next_follow_up_date || !final_follow_up_date) {
            try {
                const nbds = await getCRRs();
                ...
                console.error('Error fetching CRR for fallback dates:', err);
            }
        }
