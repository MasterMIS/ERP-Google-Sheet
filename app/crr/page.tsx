...
function NBDContent() {
    ...
    const fetchNBDs = async () => {
        try {
            ...
            if (res.ok) {
                ...
                toast.error('Failed to fetch CRR records');
            }

            // Fetch follow-up data
            const followUpRes = await fetch('/api/crr-followup');
            if (followUpRes.ok) {
    ...
    const fetchNBDHistory = async (id: number) => {
        try {
            const res = await fetch(`/api/crr-followup?id=${id}`);
            if (res.ok) {
                const data = await res.json();
                setNbdHistory(data);
            }
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (res.ok) {
                toast.success(isEditing ? 'CRR updated' : 'CRR created');
    ...
    const confirmDelete = async () => {
        ...
        try {
            loader.showLoader();
            const res = await fetch(`/api/crr?id=${deleteId}`, { method: 'DELETE' });
            if (res.ok) {
    ...
    const handleImportCSV = async (event: React.ChangeEvent<HTMLInputElement>) => {
        ...
        try {
            ...
            if (res.ok) {
                toast.success(`Successfully imported ${records.length} CRR records`);
                fetchNBDs();
            } else {
                    ...
                    <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-2">
                        ...
                        <div className="flex gap-2 flex-wrap items-center">
                            ...
                            <motion.button
                                ...
                                title="Show CRR records with today's or past follow-up dates"
                            >
                                <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            >
                                ...
                                <span className="hidden sm:inline">Add New CRR</span>
                    ...
                    >
                        {sortedNBDs.length === 0 ? (
                            <div className="p-12 text-center">
                                ...
                                <p className="text-gray-600 dark:text-gray-400 mb-4">No CRR records found</p>
                                <button
                                    onClick={() => openModal()}
                                    className="text-[var(--theme-primary)] hover:underline font-semibold"
                                >
                                    Create your first CRR
                                </button>
                            </div>
                        ) : (
                            <>
                                    ...
                                    <div className="grid grid-cols-2 gap-3">
                                        <motion.button
                                            onClick={async () => {
                                                ...
                                                try {
                                                    loader.showLoader();
                                                    const res = await fetch('/api/crr-followup', {
                                                        method: 'POST',
                                                        headers: { 'Content-Type': 'application/json' },
                                                        body: JSON.stringify({
                                                            nbd_id: selectedNBD.id,
                            ...
                            >
                                ...
                                <div className="bg-gradient-to-r from-[var(--theme-primary)] to-[var(--theme-secondary)] p-6 sticky top-0 z-10">
                                    <div className="flex justify-between items-center">
                                        <h2 className="text-2xl font-bold text-gray-900">
                                            {editingNBD ? 'Edit CRR' : 'Create New CRR'}
                                        </h2>
                                        <button
                                            onClick={() => setIsModalOpen(false)}
                                            className="p-2 hover:bg-white/20 rounded-lg transition"
                                ...
                                <form onSubmit={handleSubmit} className="p-6 space-y-5">
                                    ...
                                    >
                                        ...
                                        >
                                            {editingNBD ? 'Update CRR' : 'Create CRR'}
                                        </button>
                                        <button
                                            type="button"
                                            onClick={() => setIsModalOpen(false)}
                                    ...
                                    <div className="flex items-start gap-4 mb-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm text-gray-500 dark:text-gray-400 mt-1">
                                                Are you sure you want to delete this CRR record?
                                            </p>
                                        </div>
                                    </div>
