...
export default function Sidebar({ isOpen, setIsOpen }: SidebarProps) {
  ...
  return (
    <>
      ...
      {/* Sidebar */}
      <div
        className={`fixed left-0 top-0 h-screen bg-[#fffbf0] dark:bg-gray-800 text-gray-900 dark:text-white transition-all duration-300 z-50 ${
          isOpen ? 'w-64' : 'w-20'
      ...
      >
        ...
        <div className="p-4">
          <div className="flex items-center gap-3">
            <div className="w-12 h-12 bg-[#f4d24a] rounded-2xl flex items-center justify-center font-bold text-xl shadow-md">
              E
            </div>
          ...
          {menuItems.map((item) => (
            <Link
              ...
              className={`flex items-center gap-4 px-4 py-3 rounded-xl transition-all duration-200 ${
                isActive(item.href)
                  ? 'bg-[#f4d24a] text-gray-900 shadow-md font-semibold'
                  : 'text-gray-700 dark:text-gray-300 hover:bg-[#f4d24a]/20 dark:hover:bg-gray-700'
              }`}
        ...
        {/* Settings - Bottom */}
        <div className="absolute bottom-0 left-0 right-0 p-4 bg-[#fffbf0] dark:bg-gray-800">
          <button
            onClick={() => setIsOpen(!isOpen)}
            className="w-full flex items-center gap-4 px-4 py-3 rounded-xl text-gray-700 dark:text-gray-300 hover:bg-[#f4d24a]/20 dark:hover:bg-gray-700 transition-all"
